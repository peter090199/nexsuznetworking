...
export class CountryCodesService {
  ...
  loadCountryCodes(): Observable<any> {
    const headers = this.createHeaders();
    return this.http.get(`${_url}country_codes`, { headers }).pipe(
      catchError(error => this.handleAuthError(error))
    );
  }

   validatePhone(countryCode: any, phoneNumber: any): Observable<any> {
     const headers = this.createHeaders();
    return this.http.post(`${_url}validate_phone`, {
